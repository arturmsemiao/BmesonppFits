...
//	 std::vector<std::string> signal = {"3gauss"};
	...
	//      return;
		RooDataSet* ds_cut;
		if(doubly==0)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		if(doubly==1)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 
		if(doubly==2)ds_cut = new RooDataSet(Form("ds_cut%d",_count),"",ds, RooArgSet(*mass, *pt, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto)); 

	    //  RooRealVar* w = (RooRealVar*) data->addColumn(wFunc) ;
	//      RooDataSet wdata(data->GetName(),data->GetTitle(),data,*data->get(),0,w->GetName()) ;
				//dsMC = new RooDataSet(Form("dsMC%d",_count),"",skimtreeMC_new,RooArgSet(*mass, *pt));
		RooDataSet* dsMC_cut;
		if(doubly==0) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "Pthatweight"); 
		if(doubly==1) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "Pthatweight"); 
		if(doubly==2) dsMC_cut = new RooDataSet(Form("dsMC_cut%d",_count),"",dsMC, RooArgSet(*mass, *pt, *w, *y), Form("%s>=%f&&%s<=%f&&Bmass>%f&&Bmass<%f&&((Bpt<10&&Bpt>0&&TMath::Abs(By)>1.5)||(Bpt>10))",varExp.Data(),ptBins_check[i],varExp.Data(),ptBins_check[i+1],minhisto, maxhisto), "Pthatweight"); 
	     // return;
